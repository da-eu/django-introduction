...
class LoginAPI(View):
    def post(self, request):
        ...
        if 'username' not in body:
            return JsonResponse({'error': 'usernameフィールドが存在しません'}, status=400)
        if not isinstance(body['username'], str):
            return JsonResponse({'error': 'usernameフィールドの値には文字列を指定してください'}, status=400)

        if 'password' not in body:
            return JsonResponse({'error': 'passwordフィールドが存在しません'}, status=400)
        if not isinstance(body['password'], str):
            return JsonResponse({'error': 'passwordフィールドの値には文字列を指定してください'}, status=400)
...
class CommentAPI(LoginRequiredAPIMixin, View):
    def get(self, request):
        ...
        if min_id is not None:
            try:
                ...
                return JsonResponse({'error': 'min_idには整数を指定してください'}, status=400)
        
        if max_id is not None:
            try:
                max_id = int(max_id)
            except ValueError:
                return JsonResponse({'error': 'max_idには整数を指定してください'}, status=400)
    ...
    def post(self, request):
        ...
        # 妥当性の検証
        if 'text' not in body:
            return JsonResponse({'error': 'textフィールドが存在しません'}, status=400)
        if not isinstance(body['text'], str):
            return JsonResponse({'error': 'textフィールドの値には文字列を指定してください'}, status=400)
        if len(body['text']) < 1 or len(body['text']) > 256:
            return JsonResponse({'error': 'コメントの文字列長が不正です'}, status=400)
...
class CommentDetailAPI(LoginRequiredAPIMixin, View):
    def get(self, request, comment_id):
        # idがcomment_idのレコード取得
        try:
            comment = Comment.objects.get(id=comment_id)
        except Comment.DoesNotExist:
            return JsonResponse({'error': 'レコードが存在しません'}, status=404)
    ...
    def patch(self, request, comment_id):
        ...
        if request.user != comment.user:
            # 他のユーザーのレコードは更新不可
            return JsonResponse({'error': '他のユーザーのレコードは更新できません'}, status=403)
    ...
    def delete(self, request, comment_id):
        ...
        if request.user != comment.user:
            # 他のユーザーのレコードは削除不可
            return JsonResponse({'error': '他のユーザーのレコードは削除できません'}, status=403)
